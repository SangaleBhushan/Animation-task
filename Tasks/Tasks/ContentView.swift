...
struct ContentView: View {
    ...
    @State private var selectedNavBarItem = ""
    ...
    var body: some View {
        ZStack{
            VStack {
                ...
                BottomNavigationBarView(isPopup: $isPopup, selectedNavBarItem: $selectedNavBarItem)
            }
            ...
            if isPopup {
                VStack{
                    Text("Selected : \(selectedNavBarItem)")
                        .padding()
                        .background(.white)
                ...
                }.frame(minWidth: 300 , minHeight: 200) .background(Color.black.opacity(0.4))
                    .cornerRadius(20)
                    .transition(.scale)
            }
        }
...
struct BottomNavigationBarView: View {
    ...
    @Binding var selectedNavBarItem : String
    var body: some View {
        GeometryReader { geometry in
            HStack(spacing: geometry.size.width / 6) {
                BottomNavBarItem(iconName: "house", text: "inicio",isPopUp: $isPopup ,selectedNavItem: $selectedNavBarItem)
                BottomNavBarItem(iconName: "arrow.left.arrow.right", text: "transferencias",isPopUp: $isPopup ,selectedNavItem: $selectedNavBarItem).offset(x:15)
                BottomNavBarItem(iconName: "arrow.right.circle", text: "código cash",isPopUp: $isPopup ,selectedNavItem: $selectedNavBarItem)
            }
            .padding()
